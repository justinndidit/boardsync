...
Azurite backs profile pictures. `appsettings.Development.json` already points at it, so uploads
work locally with no extra configuration; without it the two upload endpoints answer 503 and the
...
- Set `Storage:ConnectionString` if profile pictures are wanted, and add the app's origins to the
